...
///
/// Reacts to:
///   - GridRoomManager.OnRoomsRecalculated  �� diff-based room node management + full reassignment
///   - BuildManager.OnFurniturePlaced        �� incremental furniture reparenting
/// </summary>
public class BuildHierarchyManager : MonoBehaviour
{
    ...
    [SerializeField] private Transform allBuildRoot;
    ...
    private void Awake()
    {
        ...
        _allBuildRoot = allBuildRoot != null ? allBuildRoot : new GameObject("AllBuildInstances").transform;
    ...
    /// <list type="bullet">
    ///   <item>Floor / ceiling: their occupancy cells are interior cells �� direct room lookup.</item>
    ///   <item>Walls: their occupancy cells sit on room boundaries �� check neighbour cells.</item>
    ...
    private RoomData GetRoomForStructural(PlacedBuildableData data)
    {
        ...
        for (int i = 0; i < occ.Length; i++)
        {
            ...
            // Direct check �� floor / ceiling cells ARE interior cells
            RoomData direct = rm.GetRoomAtCell(worldCell);
            if (direct != null) return direct;

            // Neighbour check �� wall cells sit outside the interior
