...
public class BuildGrid3D
{
    ...
    /// Does NOT check surface requirements �� only occupancy collisions.
    /// </summary>
    ...
    /// Does NOT modify any state �� pure read-only simulation.
    /// 
    ...
    public RemovalImpact WouldRemoveAffectOthers(string instanceId)
    {
        ...
        while (toProcess.Count > 0)
        {
            ...
            for (int s = 0; s < surf.Length; s++)
            {
                ...
                // Surface is gone at this cell �� find all occupants that require it
                foreach (var kvp in occupancyMap)
                {
                    ...
                    for (int o = 0; o < occCells.Length; o++)
                    {
                        ...
                        // This occupant depends on the lost surface �� affected
