...
/// (±X, ±Z walls + YNeg floor + YPos ceiling).
/// </summary>
public class FullEnclosurePolicy : IRoomDetectionPolicy
{
    ...
    public bool IsValidInteriorCell(Vector3Int cell, BuildGrid3D grid)
    {
        ...
        // If the cell itself (non-directional) is occupied as a wall, treat it as solid block → not interior
    ...
    /// A door counts only when it sits on the TRUE boundary of the region —
    ...
    public bool ValidateRegion(HashSet<Vector3Int> region,
        ...
        BuildGrid3D grid)
    {
        // No door requirement configured → accept any enclosure
        ...
        foreach (Vector3Int cell in region)
        {
            for (int d = 0; d < 6; d++)
            {
                ...
                // If it IS in the region, both sides are interior → this door is placed in the
...
/// A room is enclosed by a floor (YNeg face) and four horizontal walls (±X, ±Z).
...
public class FlatRoomPolicy : IRoomDetectionPolicy
{
    ...
    /// <summary>
    /// No additional validation for flat rooms — any enclosed region with a floor qualifies.
...
public class RoomData
{
    ...
    /// (min X → min Z → min Y) in the room's cell set.
    ...
    /// <summary>
    /// Returns the lexicographically smallest cell (min X → min Z → min Y)
...
/// Fully independent from <see cref="BuildManager"/> — reads the grid as a data source.
...
public class GridRoomManager : MonoBehaviour, IDebuggable
{
    ...
    /// <summary>
    /// Fired after furniture tag counts change in a room — either from an incremental
    ...
    /// Does NOT modify any state — this is a pure read-only simulation.
    /// </summary>
    public RoomBreakResult WouldRemoveBreakRoom(PlacedBuildableData data)
    {
        ...
        //    using the simulated map. If the fill escapes → room is broken.
                ...
                for (int x = min.x; x < max.x; x++)
                {
                    ...
                    while (queue.Count > 0)
                    {
                        ...
                        for (int d = 0; d < s_allDirections.Length; d++)
                        {
                            ...
                            // If we can reach outside grid bounds → this region is "outdoors"
                            if (!grid.IsInBounds(neighbor))
                            {
                                escaped = true;
