...
public class BuildManager : MonoBehaviour, IDebuggable, IGeneralPanelOwner
{
    ...
    /// Does NOT fire OnGridChanged �� room structure is unaffected by furniture.
    /// </summary>
    ...
    private void Update()
    {
        switch (CurrentState)
        {
            case BuildState.Inactive:
                return; // build system is off �� skip all input
    ...
    private void HandleIdleInput()
    {
        ...
        {
            // Track hover target change �� clear old alert highlights when moving to a different object
            ...
            if (Input.GetMouseButtonDown(0) && !IsPointerOverUI())
            {
                ...
                {
                    // canMove == false �� show disabled highlight + affected buildables on click
                ...
                else
                {
                    ...
                    {
                        // Would affect others �� show alert highlight on click
                        previewController.ShowHoverPreview(data,
        ...
        {
            // No buildable under cursor �� clear hover
...
{
    Inactive,         // build system is off �� no input processed
    Idle,
